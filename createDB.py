...
def get_post(forum_id, thread_id):
    ...
    query = "SELECT * FROM threads WHERE Id = '{}'".format(thread_id)
    
    #Get server sharding database
    post_server_id = get_server_id(thread_id)

    #Database init for thread
    dbindex = 0
    ...
    query = "SELECT post_authorId as author, post_text as text, {} as timestamp FROM posts WHERE post_threadId = '{}' AND post_forumid = {}".format(timestamp, thread_id ,str(forum_id))
    post = query_db(query, post_server_id)
    return jsonify(post)

#POST POST
@app.route('/forums/<int:forum_id>/<uuid:thread_id>', methods=['POST'])
@basic_auth.required
def post_post(forum_id, thread_id):
    # Select from forums on forum id to make sure that the forum exists
    query = 'SELECT * FROM forums WHERE id = ' + str(forum_id)
    dbindex = 0
    forum = query_db(query, dbindex)
    if len(forum) == 0:
        error = '404 No forum exists with the forum id of ' + str(forum_id)
        return make_response(jsonify({'error': error}), 404)
    # Select from threads on thread_id to make sure thread exists
...
def create_user():
    ...
    listusername = query_db(query, db_index)
...
def change_password(user):
    ...
    useracc = query_db(query,db_index)
    if not useracc:
